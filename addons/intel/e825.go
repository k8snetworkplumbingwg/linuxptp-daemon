...
func applyBcDpllPinReset(device string) {
	if device == "" {
		return
	}
	...
	for name, opts := range (*nodeProfile).Plugins {
		if name == pluginNameE825 {
			...
			if !e825Opts.Gnss.Disabled {
				applyBcDpllPinReset(leadingInterfaceForPinReset(nodeProfile, allDevices))
			}
			...
			if tbcConfigured(nodeProfile) {
				...
				applyBcDpllPinReset(nodeProfile.PtpSettings["leadingInterface"])
