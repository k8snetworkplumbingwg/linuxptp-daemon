...
func OnPTPConfigChangeE825(_ *interface{}, nodeProfile *ptpv1.PtpProfile) error {
	...
	for name, opts := range (*nodeProfile).Plugins {
		if name == pluginNameE825 { // "e825"
			...
			// Detect DPLL clock ID by module name (zl3073x)
			zlClockID, zlErr := getZL3073xClockID()
			if zlErr != nil {
				glog.Errorf("e825: failed to resolve DPLL clock ID via netlink: %v", zlErr)
			}
...
// getZL3073xClockID finds the DPLL device clock ID whose module-name is "zl3073x"
// It prefers PPS type if multiple are present but returns any matching one.
func getZL3073xClockID() (uint64, error) {
	if unitTest {
	...
	for _, d := range devices {
		if strings.EqualFold(d.ModuleName, "zl3073x") {
			if d.Type == 1 { // pps
	...
	return 0, fmt.Errorf("zl3073x DPLL not found")
}
