...
func (l *CloudEventProxy) Listen(r io.Reader) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &msg); err != nil {
			log.Errorf("IPC unmarshal error, skipping message: %v", err)
			continue
		}
		if msg.Version != ipc.Version {
			log.Warnf("IPC version mismatch: got %d, want %d", msg.Version, ipc.Version)
			continue
		}

		switch msg.Type {
		case ipc.TypeCacheClear:
			log.Info("received cache_clear from daemon, clearing event cache")
			l.cache.Clear()
			continue
		case ipc.TypeStatusResponse:
			// TODO: handle this
			log.Info("received status_response from daemon")
			continue
		case ipc.TypeStatusRequest:
			log.Warn("received status_request from daemon (unexpected direction)")
			continue
		}
	...
	if err := scanner.Err(); err != nil {
		log.Errorf("IPC reader error: %v", err)
	}
}
...
func (l *CloudEventProxy) ListenAndServe(port int) {
	...
	log.Infof("API server listening on :%d", port)
...
func (l *CloudEventProxy) currentStateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Errorf("failed to convert event to CloudEvent: %v", err)
