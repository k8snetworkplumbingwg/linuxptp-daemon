...
func (ps *PubSub) Subscribe(resource, endpoint string) string {
	...
	if err := ps.save(); err != nil {
		log.Errorf("failed to save subscriptions: %v", err)
	}
	return id
}
...
func (ps *PubSub) Publish(e *event.Event) {
	...
	if err != nil {
		log.Errorf("failed to marshal CloudEvent: %v", err)
		return
	}
	data = append(data, '\n')
	...
	for _, t := range ps.getTargets(e.Source) {
		if _, writeErr := t.w.Write(data); writeErr != nil {
			log.Errorf("failed to write event to subscriber %s: %v", t.id, writeErr)
		}
	}
}
