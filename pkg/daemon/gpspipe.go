...
func (gp *gpspipe) CmdStop() {
	...
	if gp.cmd == nil {
		glog.Infof("No command to stop for %s", gp.name)
	...
	if gp.cmd.Process != nil {
		...
		case <-time.After(5 * time.Second):
			glog.Warningf("Process %s did not terminate gracefully, sending KILL", gp.name) // this ensures that the process is terminated
			gp.cmd.Process.Signal(syscall.SIGKILL)
			<-done
		}
	}
	...
	removeErr := os.Remove(GPSPIPE_SERIALPORT)
...
func (gp *gpspipe) CmdRun(stdoutToSocket bool) {
	...
	for {
		...
		// Wait before restarting to prevent rapid restarts
		time.Sleep(1 * time.Second)
	}
}
...
func mkFifo() error {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// If this is the last attempt, handle based on environment
		if attempt == maxRetries {
			glog.Errorf("Failed to create named pipe after %d attempts. Last error: %v. GNSS monitoring will not work.", maxRetries, err)

			// In test environment, return error instead of panicking
			if os.Getenv("SKIP_GNSS_MONITORING") == "1" {
				glog.Warning("In test environment, returning error instead of panicking")
				return fmt.Errorf("failed to create named pipe after %d attempts: %v", maxRetries, err)
			}

			// In production, panic as this is critical for GNSS monitoring
			glog.Fatalf("CRITICAL: Failed to create named pipe after %d attempts: %v. GNSS monitoring is essential and cannot continue.", maxRetries, err)
		}
	...
	// This should never be reached due to panic/return above, but just in case
	if os.Getenv("SKIP_GNSS_MONITORING") == "1" {
		return fmt.Errorf("failed to create named pipe after %d attempts", maxRetries)
	}
	glog.Fatalf("CRITICAL: Failed to create named pipe after %d attempts", maxRetries)
	return fmt.Errorf("failed to create named pipe after %d attempts", maxRetries)
}
...
func removeExistingPipe() error {
	...
	if _, err := os.Stat(GPSPIPE_SERIALPORT); err == nil {
		...
		if err = os.Remove(GPSPIPE_SERIALPORT); err != nil {
