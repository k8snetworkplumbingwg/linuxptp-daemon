...
func TestMkFifoSuccess(t *testing.T) {
	...
	if removeErr := os.Remove(testGPSPipeSerialPort); removeErr != nil && !os.IsNotExist(removeErr) {
		t.Logf("Warning: failed to remove existing test named pipe: %v", removeErr)
	}
	if removeErr := os.RemoveAll(testGPSdDir); removeErr != nil && !os.IsNotExist(removeErr) {
		t.Logf("Warning: failed to remove existing test directory: %v", removeErr)
	}
	...
	// Clean up
	if removeErr := os.Remove(testGPSPipeSerialPort); removeErr != nil && !os.IsNotExist(removeErr) {
		t.Logf("Warning: failed to clean up test named pipe: %v", removeErr)
	}
	if removeErr := os.RemoveAll(testGPSdDir); removeErr != nil && !os.IsNotExist(removeErr) {
		t.Logf("Warning: failed to clean up test directory: %v", removeErr)
	}
}
...
func TestCreateNamedPipe(t *testing.T) {
	...
	createErr := testCreateNamedPipe()
	if createErr != nil {
		t.Errorf("testCreateNamedPipe failed: %v", createErr)
	}
