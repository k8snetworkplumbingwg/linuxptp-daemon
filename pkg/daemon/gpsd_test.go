...
func TestProcessGNSSLines(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gnssNotifyCh := make(chan bool, 10)
			g := &GPSD{
				gnssNotifyCh: gnssNotifyCh,
			...
			assert.Equal(t, tt.wantState, g.state)
			assert.Equal(t, tt.wantSourceLost, g.sourceLost)
			assert.Equal(t, tt.wantOffset, g.offset)

			require.Len(t, eventCh, 1)
			ev := <-eventCh
			assert.Equal(t, event.GNSS, ev.ProcessName)
			assert.Equal(t, tt.wantState, ev.State)
			assert.Equal(t, tt.wantGPSStatus, ev.Values[event.GPS_STATUS])
			assert.Equal(t, tt.wantOffset, ev.Values[event.OFFSET])
			assert.Equal(t, tt.wantSourceLost, ev.SourceLost)
			assert.Equal(t, "ens2f0", ev.IFace)

			require.Len(t, gnssNotifyCh, 1)
			assert.Equal(t, tt.wantState == event.PTP_LOCKED, <-gnssNotifyCh)
		})
	}
}
