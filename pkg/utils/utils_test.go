...
func (c testIfaceCollection) GetIfnamesForPhcID(phcId string) []string {
	return c.phcToIfnames[phcId]
}

func (c testIfaceCollection) GetPhcIDs() []string {
	ids := make([]string, 0, len(c.phcToIfnames))
	for id := range c.phcToIfnames {
		ids = append(ids, id)
	}
	return ids
}
